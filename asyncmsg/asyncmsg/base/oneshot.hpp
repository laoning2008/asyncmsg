...
namespace std{
    template<>
    struct is_error_code_enum<asyncmsg::base::errc> : true_type {};
}
        ...
        return net::async_initiate<decltype(token), void(error_code)>(
            [this](auto handler) {
                ...
                if (c_slot.is_connected()) {
                    c_slot.assign( [this](net::cancellation_type type) {
                            if (type != net::cancellation_type::none) {
                                // possible vals: waiting, sent,
                                // detached(sender)
                                auto prev = state_.exchange(empty, std::memory_order_relaxed);
